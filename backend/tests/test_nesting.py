...
def _make_sheet(width: float = 600, depth: float = 400, thickness: float = 18) -> SheetSettings:
...
class TestAutoNesting:
    def test_single_part_bottom_left(self):
        ...
        sheet = _make_sheet()
        result = auto_nesting(objects, sheet)
        assert len(result) == 1
        p = result[0]
    ...
    def test_overflow_to_second_sheet(self):
        """1枚に収まらないとき2枚目に溢れる"""
        # シートが 200x200 で、100x100 パーツを 5 個
        objects = [_make_object(f"obj_{i:03d}", 100, 100) for i in range(5)]
        sheet = _make_sheet(width=200, depth=200)
        result = auto_nesting(objects, sheet, tool_diameter=0, clearance=0)
        sheet_ids = set(p.sheet_id for p in result)
        assert len(sheet_ids) >= 2, f"Expected 2+ sheets, got {sheet_ids}"
    ...
    def test_large_part_warning(self):
        """シートより大きいパーツは sheet_1 に配置して警告対象"""
        objects = [_make_object("obj_big", 700, 500)]  # シート(600x400)より大きい
        sheet = _make_sheet()
        result = auto_nesting(objects, sheet)
        assert len(result) == 1
